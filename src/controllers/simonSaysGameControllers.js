...
import SimonSaysGame from "../models/simonSaysGameModel.js";
...
export const newGameScore = async (req, res) => {
  ...
  const [topScoreRecord] = await SimonSaysGame.aggregate([
  ...
  const game = await SimonSaysGame.create({
    playerId: userId,
    score: String(score),
  });
...
export const topTenScorers = async (req, res) => {
  const topScorers = await SimonSaysGame.aggregate([
